...
struct msg_ret_t recvMsgFromAll(void * msg){
	struct timeval timeout = {0, SELECT_TIMEOUT};
...
struct msg_ret_t recvMsgByClient(void * msg, int client_id){
	struct timeval timeout = {0, SELECT_TIMEOUT};
	fd_set readfds = active_fd_set;
	int sel_ret = select(FD_SETSIZE, &readfds, NULL, NULL, &timeout);
	if(sel_ret < 0){
	...
	if(isValidId(client_id)){
		if(FD_ISSET(connected_clients[client_id].sockid, &readfds)){
			return recvMsgFromClient(msg, client_id, WAIT_FOR_IT);
		}
	}
	return make_msg_ret(NO_MESSAGE, -1, 0);
}
